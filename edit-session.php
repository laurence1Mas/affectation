...
<?php
    ...
    $codesession=$_GET['updateid'];
    $sql="select * from `session` where codesession=$codesession";
    ...
    if(isset($_POST['updatesession'])){
        ...
        $sql=" UPDATE `session` SET `codesession`='$codesession',`designation`='$designation' WHERE `codesession`='$codesession'";
        $result=mysqli_query($con,$sql);
        if($result){
          echo "data update";
...
<button type="submit" name="updatesession" class="btn btn-info waves-effect waves-light"> Update session</button>
</form>
