...
if (isset($_POST['addpayement'])) {
    $refinscription = $_POST['refinscription'];
    $refprevision = $_POST['refprevision'];
    $montant=$_POST['montant'];
    $datepayement=$_POST['datepayement'];
    $sql = " INSERT INTO `payements`(`refinscription`, `refprevision`, `montant`,`datepayement`) VALUES ('$refinscription','$refprevision','$montant','$datepayement')";
    $result = mysqli_query($con, $sql);
    if ($result) {
        echo "data insert";
        header('location:tpayement.php');
    } else {
...
<ul class="breadcrumb">
<li class="breadcrumb-item"><a href="tpayement.php">Payement</a></li>
<li class="breadcrumb-item active">All Payement</li>
</ul>
</div>
</div>
</div>
...
<div class="col">
<h3 class="page-title">Payement</h3>
</div>
...
<h4 class="modal-title">Enregistrement Payement</h4>
...
<div class="row">
    <div class="col-md-6">
        <div class="mb-3">
            ...
            <input type="date" class="form-control" id="datepayement" name="datepayement">
        </div>
    </div>
    <div class="col-md-6">
    <div class="mb-3">
        <label for="field-3" class="form-label">Etudiant</label>
            ...
            ?>
            <select name="refinscription" id="refinscription" class="form-control">
            ...
            <label for="field-3" class="form-label">Prevision</label>
            <?php
            $sql = "SELECT * FROM `vprevision`";
            $result = mysqli_query($con, $sql);
            $row = mysqli_fetch_assoc($result);
            ?>
            <select name="refprevision" id="refprevision" class="form-control">
                <option >select option ---</option>
                <?php
                while ($row = mysqli_fetch_assoc($result)) {
                    ?>
                            <option value= "<?php echo $row["idprevision"]; ?>" ><?php echo $row["montant"]; ?> </option>;
                        <?php
                }
                ?>                                     
            </select>
...
<button type="submit" name="addpayement" class="btn btn-info waves-effect waves-light"> Add Payement</button>
</form>
</div>
...
<div class="table-responsive">
  ...
  >
    ...
    <tbody>
    <?php
    $sql = "SELECT * FROM `vpayement`";
    $result = mysqli_query($con, $sql);
    if ($result) {
        while ($row = mysqli_fetch_array($result)) {
            $odepayement = $row['idpayement'];
            $etudiant = $row['Etudiant_inscription'];
            $prevision= $row['refprevision'];
            $montant=$row['montant'];
            $datepayement = $row['datepayement'];
            echo '<tr>
                  <th scope="row">' . $odepayement . '</th>
                  <td>' . $etudiant . '</td>
                  <td>' . $prevision . '</td>
                  <td>' . $montant . '</td>
                  <td>' . $datepayement . '</td>
                  <td>
                  <a href="student-details.php?selectid=' . $odepayement . '" class="btn btn-sm bg-success-light me-2">
                      <i class="feather-eye"></i>
                  </a>
                  <button  class="btn btn-primary"> <a href="edit-payement.php?
                  updateid=' . $odepayement . '" class="text-light"><i class="feather-edit"></i></a> </button>
                  <button class="btn btn-danger"name="danger"> <a href="deletepayement.php?
                  deletedid=' . $odepayement . '" class="text-light"><i class="feather-delete"></i></a> </button>
                  </td>
                  </tr>';
        }
