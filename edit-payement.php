...
$idpayement=$_GET['updateid'];
$sql="select * from `payements` where idpayement=$idpayement";
$result=mysqli_query($con,$sql);
$row=mysqli_fetch_assoc($result);

$refinscription = $row['refinscription'];
$refprevision = $row['refprevision'];
$montant=$row['montant'];
$datepayement=$row['datepayement'];

if (isset($_POST['updatepayement'])) {
    $refinscription = $_POST['refinscription'];
    $refprevision = $_POST['refprevision'];
    $montant=$_POST['montant'];
    $datepayement=$_POST['datepayement'];
    $sql = " UPDATE `payements` SET `idpayement`='$idpayement',`refinscription`='$refinscription',`refprevision`='$refprevision',`montant`='$montant',`datepayement`='$datepayement' WHERE `idpayement`='$idpayement'";
    $result = mysqli_query($con, $sql);
    if ($result) {
        echo "data insert";
...
<div class="row">
    <div class="col-md-6">
        <div class="mb-3">
            ...
            <input type="text" name="montant" class="form-control" id="field-3" placeholder="montant payer" value=<?php echo $montant;?>>
            <label for="datepayement"> Date payement</label>
            <input type="date" class="form-control" id="datepayement" name="datepayement" value=<?php echo $datepayement;?>>
        </div>
    </div>
    <div class="col-md-6">
    <div class="mb-3">
        <label for="field-3" class="form-label">Etudiant</label>
            ...
            <select name="refprevision" id="refprevision" class="form-control">
                <option >select prevision ---</option>
                <?php
                while ($row = mysqli_fetch_assoc($result)) {
                    ?>
...
<button type="submit" name="updatepayement" class="btn btn-info waves-effect waves-light"> Update Payement</button>
</form>
