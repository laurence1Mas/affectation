...
if (isset($_POST['addcotation'])) {
    $refetudiant = $_POST['refetudiant'];
    $refcours = $_POST['refcours'];
    $reftypecote= $_POST['reftypecote'];
    $pointobtenu= $_POST['pointobtenu'];
    $sql="INSERT INTO `cotation`(`refinscription`, `refcours`, `reftypecote`, `pointobtenu`) VALUES ('$refetudiant','$refcours','$reftypecote','$pointobtenu')";
    $result = mysqli_query($con, $sql);
    if ($result) {
        echo "data insert";
        header('location:tcotation.php');
    } else {
...
<ul class="breadcrumb">
<li class="breadcrumb-item"><a href="tcotation.php">Cotation</a></li>
<li class="breadcrumb-item active">All Cotation</li>
</ul>
</div>
</div>
</div>
...
<div class="col">
<h3 class="page-title">Cotation</h3>
</div>
...
<h4 class="modal-title">Enregistrement Cotation</h4>
...
<div class="col-md-6">
    <div class="mb-3">
        <label for="field-3" class="form-label">Etudiant</label>
            ...
            <label for="field-3" class="form-label">cours</label>
            <?php
            $sql = "SELECT * FROM `cours`";
            $result = mysqli_query($con, $sql);
            $row = mysqli_fetch_assoc($result);
            ?>
            <select name="refcours" id="refcours" class="form-control">
                <option >select cours ---</option>
                <?php
                while ($row = mysqli_fetch_assoc($result)) {
                    ?>
                            <option value= "<?php echo $row["codecours"]; ?>" ><?php echo $row["designation"]; ?> </option>;
                        <?php
                }
                ?>                                     
            </select>
    ...
    <div class="col-md-6">
        <div class="mb-3">
            <label for="field-3" class="form-label">Type cote</label>
            <?php
            $sql = "SELECT * FROM `typecote`";
            $result = mysqli_query($con, $sql);
            $row = mysqli_fetch_assoc($result);
            ?>
            <select name="reftypecote" id="reftypecote" class="form-control">
                <option >select annee ---</option>
                <?php
                while ($row = mysqli_fetch_assoc($result)) {
                    ?>
                            <option value= "<?php echo $row["codetypecote"]; ?>" ><?php echo $row["designation"]; ?> </option>;
                        <?php
                }
                ?>                                     
            </select>
            <label for="pointObteneu">Point Obtenu</label>
            <input type="text" class="form-control" id="pointObteneu" name="pointobtenu">
        </div>
    </div>
</div>
<button type="submit" name="addcotation" class="btn btn-info waves-effect waves-light"> Add cotation</button>
</form>
</div>
...
<div class="table-responsive">
  ...
  >
    <thead class="student-thread">
      ...
      <th scope="col">Typecote</th>
      <th scope="col">PointObtenu</th>
      <th scope="col">Actions</th>
      </tr>
    </thead>
    <tbody>
    <?php
    $sql = "SELECT * FROM `vcotation`";
    $result = mysqli_query($con, $sql);
    if ($result) {
        while ($row = mysqli_fetch_array($result)) {
            $codecotation = $row['codecotation'];
            $etudiant = $row['etudiant'];
            $cours= $row['cours'];
            $typecote = $row['typecote'];
            $pointobtenu=$row['pointobtenu'];
            echo '<tr>
                  <th scope="row">' . $codecotation . '</th>
                  <td>' . $etudiant . '</td>
                  <td>' . $cours . '</td>
                  <td>' . $typecote . '</td>
                  <td>' . $pointobtenu . '</td>
                  <td>
                  <a href="student-details.php?selectid=' . $codecotation . '" class="btn btn-sm bg-success-light me-2">
                      <i class="feather-eye"></i>
                  </a>
                  <button  class="btn btn-primary"> <a href="edit-cotation.php?
                  updateid=' . $codecotation . '" class="text-light"><i class="feather-edit"></i></a> </button>
                  <button class="btn btn-danger"name="danger"> <a href="deletecotation.php?
                  deletedid=' . $codecotation . '" class="text-light"><i class="feather-delete"></i></a> </button>
                  </td>
                  </tr>';
        }
